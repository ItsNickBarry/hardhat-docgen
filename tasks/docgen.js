...
task('docgen', 'Generate NatSpec documentation automatically on compilation')
  ...
  .setAction(async function (args, hre) {
    ...
    for (let contractName of contractNames) {
      ...
      if (
        ...
        config.except.some((m) => contractName.match(m))
      )
        continue;
      ...
      const getSigType = function ({ type, components = [] }) {
        return type.replace(
          'tuple',
          `(${components.map(getSigType).join(',')})`,
        );
      };
      ...
      Object.keys(devdoc.stateVariables || {}).forEach(function (name) {
        Object.assign(members[`${name}()`] || {}, devdoc.stateVariables[name], {
          type: 'stateVariable',
        });
      });
      ...
      const constructor =
        members[Object.keys(members).find((k) => k.startsWith('constructor('))];
